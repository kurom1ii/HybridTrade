...
use crate::{
    ...
    models::{
        new_id, AgentMessageRow, AgentRole, AgentStatusView, Citation, CreateInvestigationRequest,
...
pub async fn finish_investigation(
    pool: &SqlitePool,
    investigation_id: &str,
    summary: &str,
    final_report: &str,
...
pub async fn set_section_status(pool: &SqlitePool, section_id: &str, status: &str) -> Result<()> {
    query("UPDATE investigation_sections SET status = ?2, updated_at = ?3 WHERE id = ?1")
        .bind(section_id)
        .bind(status)
        .bind(now_rfc3339())
        .execute(pool)
...
pub async fn conclude_section(pool: &SqlitePool, section_id: &str, conclusion: &str) -> Result<()> {
    query(
        "UPDATE investigation_sections SET status = 'concluded', conclusion = ?2, updated_at = ?3 WHERE id = ?1",
    )
    .bind(section_id)
    .bind(conclusion)
    .bind(now_rfc3339())
...
pub async fn insert_message(
    ...
    query(
        r#"
        INSERT INTO agent_messages (id, investigation_id, section_id, agent_role, target_role, kind, content, citations_json, confidence, created_at)
    ...
    .bind(target_role.map(AgentRole::as_str))
    .bind(kind)
    .bind(content)
    .bind(to_json_string(citations))
    .bind(confidence)
    .bind(now)
    .execute(pool)
    .await?;

    query_as::<_, AgentMessageRow>("SELECT * FROM agent_messages WHERE id = ?1")
        .bind(id)
        .fetch_one(pool)
        .await
        .context("cannot fetch inserted message")
}
...
pub async fn insert_finding(
    ...
    query(
        r#"
        INSERT INTO findings (id, investigation_id, section_id, agent_role, kind, title, summary, direction, confidence, evidence_json, created_at)
    ...
    .bind(direction)
    .bind(confidence)
    .bind(to_json_string(evidence))
    .bind(now)
    .execute(pool)
    .await?;

    query_as::<_, FindingRow>("SELECT * FROM findings WHERE id = ?1")
        .bind(id)
        .fetch_one(pool)
        .await
        .context("cannot fetch inserted finding")
}
...
pub async fn insert_source_document(
    ...
    excerpt: Option<&str>,
    metadata: Value,
) -> Result<SourceDocumentRow> {
    let id = new_id();
    let now = now_rfc3339();
    query(
        r#"
        INSERT INTO source_documents (id, investigation_id, url, title, fetched_at, excerpt, metadata_json)
    ...
    .bind(now)
    .bind(excerpt)
    .bind(to_json_string(&metadata))
    .execute(pool)
    .await?;

    query_as::<_, SourceDocumentRow>("SELECT * FROM source_documents WHERE id = ?1")
        .bind(id)
        .fetch_one(pool)
        .await
        .context("cannot fetch inserted source")
}
...
pub async fn start_agent_run(
    ...
    query(
        r#"
        INSERT INTO agent_runs (id, investigation_id, agent_role, status, started_at, completed_at)
...
pub async fn finish_agent_run(pool: &SqlitePool, run_id: &str) -> Result<()> {
    query("UPDATE agent_runs SET status = 'completed', completed_at = ?2 WHERE id = ?1")
        .bind(run_id)
        .bind(now_rfc3339())
        .execute(pool)
        .await?;
