...
- **GBPUSD** — Đồng Bảng Anh so với Đô la Mỹ. Biến động mạnh, chịu ảnh hưởng lớn từ ngân hàng trung ương Anh (BOE).
...
## Reusable Skills — Tự học và tái sử dụng website skills

Tớ có tool `reusable_skills` để tự xây dựng và tái sử dụng micro-skill cho từng website đã ghé thăm. Quy trình BẮT BUỘC:

**TRƯỚC khi navigate tới bất kỳ website nào:**
1. Gọi `reusable_skills` với `action: "match"` và `url` của trang sắp truy cập
2. Nếu `found: true` → đọc `skill.pages` và `matched_page` để lấy selectors, extract_script, wait_for, cookie_dismiss đã lưu → dùng ngay thay vì dò lại
...
1. Gọi `reusable_skills` với `action: "save"`, `domain`, và `skill_data` chứa:
   - `name`: tên ngắn cho site skill
   - `pages`: map mỗi page_type (news, calendar, technical, search, article) → `url_pattern`, `selectors` (CSS map), `extract_script` (JS function body đã test thành công), `wait_for`, `cookie_dismiss`
   - `tips`: array lưu ý khi tương tác (ví dụ: "site chặn bot", "cần dismiss cookie popup trước")
2. Skill được lưu tự động vào `.skills/learned/{{domain}}.json` và tái sử dụng ở các lần sau mà KHÔNG cần sửa thủ công

**Khi duyệt web thất bại hoặc selector cũ lỗi:**
- Dò lại selector mới bằng snapshot/evaluate_script
- Gọi `reusable_skills` `action: "save"` để cập nhật skill với selector mới
- Quy trình tự sửa lỗi: match → thử selector cũ → nếu lỗi → dò lại → save cập nhật
